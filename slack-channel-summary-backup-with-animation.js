...
(function() {
    ...
    const SlackChannelSummary = {
        ...
        // Start loading animation for channel summary generation
        startLoadingAnimation: function(textbox) {
            const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
            let frameIndex = 0;
            ...
            this.loadingInterval = setInterval(() => {
                ...
                textbox.value = `${spinner} Fetching channel messages...\n\n` +
                    `Please wait while we gather the relevant messages from this channel.\n` +
                    `This may take a few moments depending on the number of messages and your selected time range.`;
        ...
        generateChannelSummary: async function(summaryWindow) {
            ...
            this.startLoadingAnimation(textbox);
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        if (timeRange !== 'all' && dateRange.startDate && dateRange.endDate) {
                            ...
                            utils.debug('🚨 DEBUG: Message filtering', {
                                timeRange: timeRange,
                                totalMessagesBeforeFilter: result.messages.length,
                                startDate: dateRange.startDate,
                                endDate: dateRange.endDate,
                                startTime: startTime,
                                endTime: endTime,
                                sampleMessageTimestamp: result.messages[0]?.timestamp,
                                sampleMessageTime: result.messages[0]?.timestamp ? new Date(result.messages[0].timestamp).getTime() : null
                            });
                            ...
                            result.messages = result.messages.filter(msg => {
                                ...
                                const isInRange = msgTime >= startTime && msgTime <= endTime;
                                return isInRange;
                            });

                            utils.debug('🚨 DEBUG: Filtering results', {
                                originalCount: originalCount,
                                filteredCount: result.messages.length,
                                messagesRemoved: originalCount - result.messages.length
                            });
                ...
                // Stop loading animation and reset button state
        ...
        processAndDisplaySummary: async function(result, summaryLevel, textbox, generateBtn) {
            try {
                ...
                // Generate AI summary if we have messages (keep animation running)
